...
class SignInImageGenerator:
    ...
    async def generate_sign_in_image(self, data: SignInData, yiyan: str) -> io.BytesIO:
        """生成签到图片 - 仿照参考布局"""
        # 创建背景
        bg_img = self._create_background()
        ...
        # 获取用户昵称或ID
        user_name = f"@{data.user_id}" if data.user_id else "@用户"
        ...
        generate_bbox = draw.textbbox((0, 0), generate_text, font=tiny_font)
        generate_width = generate_bbox[2] - generate_bbox[0]
        generate_x = (self.width - generate_width) // 2
        generate_y = self.height - 50
        draw.text((generate_x, generate_y), generate_text, fill=(150, 150, 150), font=tiny_font)
...
async def create_sign_in_image(sign_rank: int, today_gold: int, 
                        ...
                        today_lucky: int = 0, user_id: Optional[int] = None, yiyan: str = "") -> io.BytesIO:
    """
    创建签到图片的便捷函数
    ...
    return await generator.generate_sign_in_image(data, yiyan)
